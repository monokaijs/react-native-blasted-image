...
@implementation BlastedImageModuleImpl
{
    BOOL hasListeners;
}
...
- (NSURL *)prepareUrl:(NSString *)imageUrl
    ...
    if (fileExistsInAssets && hybridAssets) {
        ...
        NSString *resourcePath = [[NSBundle mainBundle] pathForResource:imagePath ofType:nil];
        url = [NSURL fileURLWithPath:resourcePath];
        if (showLog) {
            [self sendEventWithName:@"BlastedEventLog" message:[NSString stringWithFormat:@"URL: %@", url]];
        }
    } else {
        if (hybridAssets) {
            if (showLog) {
...
{
    return std::make_shared<facebook::react::NativeBlastedImageSpecJSI>(params);
}
